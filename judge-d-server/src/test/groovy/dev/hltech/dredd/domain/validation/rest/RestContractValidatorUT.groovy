...
class RestContractValidatorUT extends Specification {
    ...
    def "should convert rawExpectations to pact"(){
        given:
            def pactAsString = new String(toByteArray(getClass().getResourceAsStream("/pact-frontend-to-dde-instruction-gateway.json")))
        when:
    ...
    def 'should return validation result with all interactions validated when validate swagger with pact' (){
        given:
            def swagger = new String(toByteArray(getClass().getResourceAsStream("/dde-instruction-gateway-swagger.json")))
            def pact = (RequestResponsePact)loadPact(getClass().getResourceAsStream("/pact-frontend-to-dde-instruction-gateway.json"))
        when:
        ...
        then:
            ...
            with(validationResult.get(0)) {
                status == FAILED
                name == "a request for instruction details"
                errors.size() == 2
            }
    }
}
