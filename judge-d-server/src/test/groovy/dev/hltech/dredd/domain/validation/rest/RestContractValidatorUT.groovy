...
class RestContractValidatorUT extends Specification {
    ...
    def 'should not return validation errors when validate given consumer ignores body'() {
        given:
            def swagger = new String(toByteArray(getClass().getResourceAsStream("/swagger-deploy-sphere-api.json")))
            def pact = (RequestResponsePact) loadPact(getClass().getResourceAsStream("/pact-deploy-sphere-agent-deploy-sphere-api.json"))
        when:
            def validationResult = validator.validate(pact, swagger)
        then:
